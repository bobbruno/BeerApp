...
def get_nearest(userPos, nBeers=10):
    ...
    print type(dfBeerData.index[theBeers[0]].values)
    print dfBeerData.index[theBeers[0]].shape
    print type(dists)
    print dists.shape
    BeerList = np.concatenate((dfBeerData.index[theBeers[0]].values, dists), axis=1).sort
    ranks = {}
