...
stops = {cmystem(x) for x in stops}.union(set([u'buddy', u'iphone', u'oz', u'ml', u'pour', u'one',
...
#  Also for particles
appColorBase = {cmystem(x) for x in [u'color', u'colour', ]}
colors = {cmystem(x) for x in [u'brown', u'yellow', u'orange', u'amber', u'red', u'ruby',
                               u'tan', u'black', u'tawny', u'beige', u'gold', u'golden', u'copper']}
colorDesignators = {cmystem(x) for x in [u'light', u'dark', u'deep', u'cristaline', u'hazy']}
appHeadBase = {cmystem(x) for x in [u'head', ]}
appHeadWords = {cmystem(x) for x in [u'spare', u'small', u'average', u'large', u'huge', u'rocky', u'dense',
                                     u'creamy', u'frothy', u'fizzy', u'white', u'off-white', u'light', u'brown',
                                     u'dark', u'lasting', u'diminishing']}
appLacingBase = {cmystem(x) for x in [u'lacing', ]}
appLacingWords = {cmystem(x) for x in [u'excellent', u'good', u'fair', u'spare']}
appBodyBase = {cmystem(x) for x in [u'body', ]}
appBodyWords = {cmystem(x) for x in [u'body', u'clear', u'sparkling', u'flat', u'cloudy', u'hazy', u'murky', u'muddy']}
appParticleBase = {cmystem(x) for x in [u'particle', ]}
appParticleWords = {cmystem(x) for x in [u'light', u'cloudy', u'heavily', u'particulate', u'chunky']}
...
#  Aroma Attributes
aromaBase = {cmystem(x) for x in [u'aroma', 'nose', u'smell']}
aroDesignators = {cmystem(x) for x in [u'toasted', u'roasted', u'burnt', u'light', u'average', u'heavy', u'harsh']}
aroMaltWords = {cmystem(x) for x in [u'malt', u'malty', u'barley', u'corn', u'bread', u'cookie', u'molasses', u'caramel',
...
aroHopsWords = {cmystem(x) for x in [u'hops', u'hoppy', u'flowers', u'floral', u'perfume', u'herbs', u'celery', u'grass', u'pine',
...
aroYeastWords = {cmystem(x) for x in [u'yeast', u'bacteria', u'dough', u'sweat', u'horse blanket', u'barnyard',
...
aroMiscWords = {cmystem(x) for x in [u'alcohol', u'banana', u'bubblegum', u'clove', u'grape', u'raisin', u'plum',
...
#  Palate Attributes
palateBase = {cmystem(x) for x in [u'finish', u'aftertaste', u'palate']}
palBodyWords = {cmystem(x) for x in [u'light', u'medium', u'full']}
palTextureWords = {cmystem(x) for x in [u'thin', u'oily', u'creamy', u'sticky',
                                        u'slick', u'alcoholic', u'thick', u'booze', u'minerals', u'gritty']}
palCarbonationWords = {cmystem(x) for x in [u'carbonation', u'fizzy', u'lively', u'average', u'soft', u'flat']}
palFinishWords = {cmystem(x) for x in [u'metallic', u'chalky', u'astringent', u'bitter', u'mouthfeel']}
...
flvDurationWords = {cmystem(x) for x in [u'short', u'average', u'long']}
flvDescriptors = {cmystem(x) for x in [u'light', u'moderate', u'heavy', u'harsh']}
flvTypeWords = {cmystem(x) for x in [u'sweet', u'acidic', u'dry', u'bitter', u'vinegar', u'salty', u'minerals', u'sour', u'tart']}
flvGeneralWords = {cmystem(x) for x in [u'flavor', u'flavour', u'taste']}
...
cdef unicode cmystem(unicode x):
    if (x == u'nutty'):
        return u'nut'
    if (x == u'reddish'):
        return u'red'
    ...
    if (x == u'cristaline'):
        return u'cristal'
    if (u' ' in x or x in [u'red', u'cereal']):
        return x
    return stemmer.stem(x)
...
cdef bint cusesVocab(list wordList, set vocabSet, float cutOff=0.1):
    """ Checks if a word list is actually using a specific vocabulary. If at least cutOff
    words in the list are contained in that vocabulary.
    :param wordList: The actual list of words to be checked
    :type wordList: list
...
cdef list cprocessBiGrams(list wordList, set biGramSet):
    """ Transforms all BiGrams in word list from pairs of tokens into single tokens.
    :param wordList: the list of tokens
    :type wordList: list
    :param biGramSet: the set of biGrams to evaluate against. They are strings, not tuples
...
cdef bint cisQuantityOrAmount(unicode w):
    """ Determines if a token is a quantity or amount
    :param w: the word to be checked
    :type w: unicode
    """
...
cdef list cremoveVocab(list wordList, set vocab):
    """ Remove all the words on vocab from the wordList.
    :param wordList: the text to process, as a list of strings
    :type wordList: list
    :param vocab: the vocabulary to be removed, as a set of tokens
    :type vocab: set
    :rtype: list """
    return [w for w in wordList if (w not in vocab or cisQuantityOrAmount(w) or len(w) < 2)]


def tokenizer(unicode d):
    cdef list l
    if not len(d):
        return []
    l = [cmystem(x) for x in utils.tokenize(d)]
    if cusesVocab(l, beerVocab):
        return cremoveVocab(cprocessBiGrams(l, beerNGrams), toRemove)
    else:
        return []
